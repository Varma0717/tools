...
class OpenAIClient:
    def __init__(self):
        ...
        if api_keys:
            ...
            self.api_key = None
            self.client = None
            print("Warning: OPENROUTER_API_KEYS not found in environment variables")

    def chat_completion(self, prompt, max_tokens=450, temperature=0.7, top_p=0.95):
        if not self.client:
            return {"error": "OpenAI client not configured. Please set OPENROUTER_API_KEYS environment variable."}
        
        try:
            messages = [{"role": "user", "content": prompt}]
            response = self.client.chat.completions.create(
                extra_headers={"HTTP-Referer": "https://superseotoolkit.com", "X-Title": "Super SEO Toolkit",},
            ...
            return {"error": f"OpenAI API error: {str(e)}"}.strip()
