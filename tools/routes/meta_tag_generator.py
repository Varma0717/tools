...
class MetaTagGenForm(FlaskForm):
    url = StringField('Website URL', validators=[Optional(), URL(message="Enter a valid URL.")])
    title = StringField('Page Title', validators=[Optional(), Length(max=70)])
    description = TextAreaField('Meta Description', validators=[Optional(), Length(max=160)])
    keywords = StringField('Keywords (comma separated)', validators=[Optional(), Length(max=255)])
...
def meta_tag_generator():
    ...
    return render_template('tools/meta_tag_generator.html', form=form, csrf_token=generate_csrf())
...
def meta_tag_generator_ajax():
    ...
    if url and not (title or description or keywords):
        try:
            ...
            keywords = key_tag['content'].strip() if key_tag and key_tag.get('content') else ""
        except Exception as e:
            return jsonify({'success': False, 'error': f"Failed to fetch data from URL: {e}"}), 500

    # Ensure at least one tag is provided
    if not (title or description or keywords):
        return jsonify({'success': False, 'error': 'Please provide at least one field or a valid URL.'}), 400
